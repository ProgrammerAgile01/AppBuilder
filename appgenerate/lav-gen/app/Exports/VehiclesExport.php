    ...
    FromCollection, WithHeadings, WithMapping, WithEvents, WithCustomStartCell, ShouldAutoSize
{
    ...
    public function collection(): Collection
    {
        ...
        if ($this->search) {
            ...
            $q->where(function($qq) use ($s) {
                $cols = DB::getSchemaBuilder()->getColumnListing('vehicles');
                foreach ($cols as $c) $qq->orWhere($c, 'like', $s);
            });
        }

        if ($this->status && DB::getSchemaBuilder()->hasColumn('vehicles', $this->statusColumn())) {
    ...
    {
        return ['Model', 'Plate Number', 'Year', 'Color', 'Location', 'Vehicle Type', 'Status', 'Daily Rate'];
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $e) {
                ...
                $s->setCellValue('A2', 'Vehicle Management - Export');
    ...
    $col = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex(8);
    ...
    ->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_RIGHT);
                ...
                $s->setCellValue("A".($sumTitle+1), 'Total Vehicle');
                $s->setCellValue("B".($sumTitle+1), $total);

                $statusCol = $this->statusColumn();
                if ($statusCol && \Schema::hasColumn('vehicles', $statusCol)) {
