...
import {
  ...
  DataPegawaiTable,
  ResultsInfo,
} from "./data-pegawai-page-contents";
...
export function DataPegawaiPage() {
  ...
  const refetch = useCallback(
    async (signal?: AbortSignal) => {
      ...
      try {
        ...
        //   fetchData("data-pegawais", { signal }),
        //   fetchModuleStats({ signal }),
        // ]);
        const dataItems = await fetchData("data-pegawais", { signal });
        ...
        toast({
          ...
          description:
            err?.message || "Terjadi kesalahan saat mengambil data dari server",
          variant: "destructive",
        });
      } finally {
  ...
  const filteredDataPegawai = useMemo(() => {
    ...
    const SEARCH_KEYS = ["nama", "alamat"] as const;
  ...
  const paginatedItems = filteredDataPegawai.slice(
    startIndex,
  ...
  const handleDelete = async (id: string) => {
    try {
      await deleteData("data-pegawais", id);
      toast({
        title: "Berhasil!",
        description: "Data DataPegawai berhasil dihapus.",
      });
  ...
  const handleView = (id: string) => {
    router.push(`/data-pegawai/view/${id}`);
  };
