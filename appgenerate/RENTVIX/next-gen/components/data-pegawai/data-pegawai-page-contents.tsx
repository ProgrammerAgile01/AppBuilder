...
// Header
export function DataPegawaiHeader({
  onAdd,
  search,
  status,
}: {
  ...
  const handleExportExcel = async () => {
    try {
      setIsExporting(true);
      await downloadExcel("data-pegawais", {
  ...
  const handleExportPDF = async () => {
    try {
      setIsExporting(true);
      await downloadPdf("data-pegawais", {
    ...
    <div className="sticky top-16 z-10 w-full bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 pb-4 -mx-4 px-4 pt-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Ini data pengelolaan pegawai</p>
        </div>
...
// Filter
export function DataPegawaiFilters({
  searchTerm,
  setSearchTerm,
  // statusFilter,
  // setStatusFilter,
  ...
  return (
    <div className="flex justify-between items-center">
      <p className="text-sm text-muted-foreground">
        Showing {startIndex + 1}-{Math.min(startIndex + itemsPerPage, total)} of {total} Data Pegawai
      </p>
    </div>
  );
}
...
// Mobile View
function MobileDataPegawaiCard({
  filteredDataPegawai,
  handleDelete,
  handleView,
}: {
  filteredDataPegawai: Record<string, any>;
    ...
    <Card className="mb-4">
      <CardContent className="p-4">
        <div className="flex gap-4">
          <Image
            src={filteredDataPegawai._url || "/placeholder.svg"}
            alt={`${filteredDataPegawai.nama}`}
            width={128}
            height={96}
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-1">
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => handleView(filteredDataPegawai.id)}
                >
                  <Eye className="h-3 w-3" />
                </Button>
                <Link href={`/data-pegawai/edit/${filteredDataPegawai.id}`}>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription className="text-muted-foreground">
                        Are you sure you want to delete {filteredDataPegawai.nama}?
                    ...
                    <AlertDialogFooter>
                      ...
                      <AlertDialogAction
                        onClick={() => handleDelete(filteredDataPegawai.id)}
                      >
                        Delete
                      </AlertDialogAction>
                    </AlertDialogFooter>
...
// desktop View
export function DataPegawaiTable({
  handleView,
  handleDelete,
  filteredDataPegawai
}: {
  ...
  return isMobile ? (
    <div>
      {filteredDataPegawai.map((item: any) => (
        <MobileDataPegawaiCard
          key={item.id}
          filteredDataPegawai={item}
    ...
    <Card>
      <CardContent className="p-0">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead className='text-foreground'>Nama Pegawai</TableHead>
          ...
          <TableBody>
            { filteredDataPegawai.map((item: any) => (
              <TableRow key={item.id}>
                <TableCell className='text-left'><div className='text-sm font-normal text-gray-900 dark:text-foreground'>{item.nama}</div>
</TableCell>
<TableCell className='text-left'><div className='text-sm font-normal text-gray-900 dark:text-foreground'>{item.alamat}</div>
                ...
                <TableCell className="text-right">
                  <div className="flex justify-end gap-2">
                    ...
                    {/* edit */}
                    <Link href={`/data-pegawai/edit/${item.id}`}>
                      <Button variant="outline" size="sm">
