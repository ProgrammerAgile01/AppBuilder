...
// Header
export function DataCustomerHeader({
  onAdd,
  search,
  status,
  onOpenTrash,
            ...
            >
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-gray-500">
                    Upload data Data Customer bulk
                  </div>
                </div>
              </DropdownMenuItem>
            </DropdownMenuContent>
  ...
  return (
    <div className="flex justify-between items-center">
      <p className="text-sm text-muted-foreground">
        Showing {_from}-{_to}
        {typeof total === "number" ? ` of ${total}` : ""} Data Customer
      </p>
    </div>
  );
}
...
// Mobile View
function MobileDataCustomerCard({
  filteredDataCustomer,
  handleDelete,
  handleView,
}: {
  filteredDataCustomer: Record<string, any>;
    ...
    <Card className="mb-4">
      <CardContent className="p-4">
        <div className="flex gap-4">
          <Image
            src={filteredDataCustomer._url || "/placeholder.svg"}
            alt={`${filteredDataCustomer.nama_lengkap}`}
            width={128}
            height={96}
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-1">
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => handleView(filteredDataCustomer.id)}
                >
                  <Eye className="h-3 w-3" />
                </Button>
                <Link href={`/data-customer/edit/${filteredDataCustomer.id}`}>
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                      ...
                      <AlertDialogAction
                        onClick={() => handleDelete(filteredDataCustomer.id)}
                      >
                        Delete
                      </AlertDialogAction>
                    </AlertDialogFooter>
  ...
  return isMobile ? (
    <div>
      {filteredDataCustomer.map((item: any) => (
        <MobileDataCustomerCard
          key={item.id}
          filteredDataCustomer={item}
...
<TableHead className='text-foreground'>Instansi</TableHead>
...
<TableCell className='text-left'><div className='text-sm font-normal text-gray-900 dark:text-foreground'>{item.nama_instansi}</div>
</TableCell>
                ...
                <TableCell className="text-right">
                  <div className="flex justify-end gap-2">
                    ...
                    {/* edit */}
                    <Link href={`/data-customer/edit/${item.id}`}>
                      <Button variant="outline" size="sm">
