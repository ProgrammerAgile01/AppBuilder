...
export function DataKendaraanHeader({
  ...
  const handleExportExcel = async () => {
    try {
      setIsExporting(true);
      await downloadExcel("data-kendaraans", {
  ...
  const handleExportPDF = async () => {
    try {
      setIsExporting(true);
      await downloadPdf("data-kendaraans", {
  ...
  return (
    <div className="flex justify-between items-center">
      <p className="text-sm text-muted-foreground">
        Showing {startIndex + 1}-{Math.min(startIndex + itemsPerPage, total)} of {total} Data Kendaraan
      </p>
    </div>
  );
}
...
function MobileDataKendaraanCard({
  ...
  return (
    ...
    <span className="text-gray-700 dark:text-foreground">{filteredDataKendaraan.lokasi}</span>
</div><div className="flex items-center gap-1">
    <Calendar className="h-3 w-3 text-gray-700 dark:text-foreground" />
    <span className="text-gray-700 dark:text-foreground">{filteredDataKendaraan.tahun}</span>
</div>
            </div>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription className="text-muted-foreground">
                        Are you sure you want to delete {filteredDataKendaraan.model}?
          ...
          <TableBody>
            { filteredDataKendaraan.map((item: any) => (
              <TableRow key={item.id}>
                <TableCell className='text-left'><img src={item.foto_depan_url} alt='' className='w-[64px] h-[64px] object-cover rounded-[0.5rem]' />
</TableCell>
<TableCell className='text-left'><div className='text-sm font-semibold text-gray-900 dark:text-foreground'>{item.model}</div>
</TableCell>
<TableCell className='text-left'><div className='text-sm font-normal text-blue-600 dark:text-foreground'>{item.lokasi}</div>
</TableCell>
<TableCell className='text-left'>{(() => {
    switch (item.status) {
        case 'Avaliable': return <Badge className='bg-green-100 text-green-700'>Avaliable</Badge>;
