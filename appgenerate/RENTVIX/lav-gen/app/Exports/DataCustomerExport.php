...
class DataCustomerExport implements
    FromCollection, WithHeadings, WithMapping, WithEvents, WithCustomStartCell, ShouldAutoSize
{
    ...
    {
        $q = DB::table('mst_customer');

        if ($this->search) {
            $s = "%{$this->search}%";
            $q->where(function($qq) use ($s) {
                $cols = DB::getSchemaBuilder()->getColumnListing('mst_customer');
                foreach ($cols as $c) $qq->orWhere($c, 'like', $s);
            });
        }

        if ($this->status && DB::getSchemaBuilder()->hasColumn('mst_customer', $this->statusColumn())) {
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $e) {
                ...
                $s->setCellValue('A2', 'Data Customer - Export');
                ...
                $total = DB::table('mst_customer')->count();
                $s->setCellValue("A".($sumTitle+1), 'Total Data Customer');
                $s->setCellValue("B".($sumTitle+1), $total);

                $statusCol = $this->statusColumn();
                if ($statusCol && \Schema::hasColumn('mst_customer', $statusCol)) {
                    $rows = DB::table('mst_customer')
