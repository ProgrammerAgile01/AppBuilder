    ...
    FromCollection, WithHeadings, WithMapping, WithEvents, WithCustomStartCell, ShouldAutoSize
{
    ...
    {
        $q = DB::table('mst_pegawai');

        if ($this->search) {
            $s = "%{$this->search}%";
            $q->where(function($qq) use ($s) {
                $cols = DB::getSchemaBuilder()->getColumnListing('mst_pegawai');
                foreach ($cols as $c) $qq->orWhere($c, 'like', $s);
            });
        }

        if ($this->status && DB::getSchemaBuilder()->hasColumn('mst_pegawai', $this->statusColumn())) {
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $e) {
                ...
                $s->setCellValue('A2', 'Data Pegawai - Export');
                ...
                $total = DB::table('mst_pegawai')->count();
                $s->setCellValue("A".($sumTitle+1), 'Total Data Pegawai');
                $s->setCellValue("B".($sumTitle+1), $total);

                $statusCol = $this->statusColumn();
                if ($statusCol && \Schema::hasColumn('mst_pegawai', $statusCol)) {
                    $rows = DB::table('mst_pegawai')
