...
class DaftarKendaraanExport implements
    FromCollection, WithHeadings, WithMapping, WithEvents, WithCustomStartCell, ShouldAutoSize
{
    ...
    {
        $q = DB::table('mst_daftar_kendaraan');

        if ($this->search) {
            $s = "%{$this->search}%";
            $q->where(function($qq) use ($s) {
                $cols = DB::getSchemaBuilder()->getColumnListing('mst_daftar_kendaraan');
                foreach ($cols as $c) $qq->orWhere($c, 'like', $s);
            });
        }

        if ($this->status && DB::getSchemaBuilder()->hasColumn('mst_daftar_kendaraan', $this->statusColumn())) {
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $e) {
                ...
                $s->setCellValue('A2', 'Daftar Kendaraan - Export');
                ...
                $total = DB::table('mst_daftar_kendaraan')->count();
                $s->setCellValue("A".($sumTitle+1), 'Total Daftar Kendaraan');
                $s->setCellValue("B".($sumTitle+1), $total);

                $statusCol = $this->statusColumn();
                if ($statusCol && \Schema::hasColumn('mst_daftar_kendaraan', $statusCol)) {
                    $rows = DB::table('mst_daftar_kendaraan')
