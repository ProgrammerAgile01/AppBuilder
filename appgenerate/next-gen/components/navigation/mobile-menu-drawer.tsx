...
/* ====== Types (kompatibel dengan props lama) ====== */
...
export function MobileMenuDrawer({
  ...
  const groups = React.useMemo(() => {
    ...
    for (const m of items) {
      const gid = String(m.groupId ?? m.groupName ?? "0");
      const name = String(m.groupName ?? "Kelompok");
      if (!map.has(gid)) {
        map.set(gid, {
          id: gid,
          name,
  ...
  /* ---- FIX HOOKS: state expand untuk nestedItems pakai map global ---- */
  ...
  /* ---- Util untuk deduplikasi item modul vs nested ---- */
  const norm = (s?: string) =>
    String(s ?? "")
      .toLowerCase()
      .replace(/\s+/g, "")
    ...
    <Drawer open={open} onOpenChange={setOpen}>
      <DrawerContent className="lg:hidden max-h[80vh] max-h-[80vh] pb-[env(safe-area-inset-bottom)] bg-background text-foreground">
        ...
        <div className="px-4 pb-4 pt-3 overflow-y-auto space-y-3">
          ...
          {/* Level 2: ITEMS / NESTED IN MODULE (dengan DEDUP) */}
          {currentModule && (
            <div className="space-y-3">
              {(() => {
                // Kunci-kunci item yang sudah ada di nested
                ...
                // Filter items di modul agar tidak duplikat
