  ...
  return (
    <div className="flex justify-between items-center">
      <p className="text-sm text-muted-foreground">
        Showing {startIndex + 1}-{Math.min(startIndex + itemsPerPage, total)} of {total} Level User
      </p>
    </div>
  );
}
...
// Mobile View (SOON)
{/* export function MobileLevelUserCard({
  levelUser,
  handleDelete,
  handleView,
}: {
  levelUser: Record<string, any>;
          ...
          <TableBody>
            { filteredLevelUser.map((item: any) => (
              <TableRow key={item.id}>
                <TableCell>{item.nama_level}</TableCell>
