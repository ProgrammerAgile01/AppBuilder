...
export function TopNavbar() {
  ...
  const handleLogout = () => {
    ...
    // Alternative: window.location.reload() if using the same page login
  }
    ...
    <header className="sticky top-0 z-50 w-full border-b border-border bg-background/95 backdrop-blur-xl supports-[backdrop-filter]:bg-background/80 shadow-sm">
      <div className="flex h-16 items-center gap-4 px-4">
        ...
        <div className="hidden md:flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/20">
          ...
          <span className="text-xs font-medium text-primary font-space-grotesk">AI-Powered</span>
        </div>
        ...
        <div className="hidden md:flex items-center gap-1">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="ghost"
                size="icon"
                className="h-9 w-9 text-foreground hover:text-primary hover:bg-accent border border-foreground/20 hover:border-primary/30 transition-all duration-300"
              >
                <Palette className="h-4 w-4" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-48 bg-popover border-border">
              <DropdownMenuLabel className="font-manrope text-popover-foreground flex items-center gap-2">
                <Palette className="h-4 w-4" />
              ...
              {themes.map((theme) => (
                <DropdownMenuItem
                  ...
                  className="font-manrope text-popover-foreground hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground flex items-center gap-3"
                >
                  ...
                  <span className={currentTheme.id === theme.id ? "text-primary font-medium" : ""}>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="ghost"
                size="icon"
                className="h-9 w-9 text-foreground hover:text-primary hover:bg-accent border border-foreground/20 hover:border-primary/30 transition-all duration-300 flex items-center justify-center"
              >
                <Globe className="h-4 w-4" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="dropdown-content font-manrope">
              <DropdownMenuItem onClick={() => setLanguage("en")}>
                <span className={language === "en" ? "font-medium text-primary" : ""}>English</span>
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setLanguage("id")}>
                <span className={language === "id" ? "font-medium text-primary" : ""}>Bahasa Indonesia</span>
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="ghost"
                className="relative h-9 w-9 rounded-full text-foreground hover:text-primary hover:bg-accent border border-foreground/20 hover:border-primary/30 transition-all duration-300"
              >
            ...
            <DropdownMenuContent align="end" className="dropdown-content w-56 font-manrope">
              <DropdownMenuLabel>
                <div className="flex flex-col space-y-1">
                  <p className="text-sm font-medium">Admin User</p>
                  <p className="text-xs text-muted-foreground">admin@rentvix.com</p>
                </div>
              </DropdownMenuLabel>
              <DropdownMenuSeparator />
              <DropdownMenuItem>
        ...
        <div className="md:hidden border-t border-border bg-background/95 backdrop-blur-xl">
          <div className="p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                {themes.map((theme) => (
                  ...
                  >
                    ...
                    <span className="text-xs">{language === "id" ? theme.nameId : theme.name}</span>
                  </Button>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                <Globe className="h-4 w-4" />
                <span className="text-xs">{language === "en" ? "ID" : "EN"}</span>
              </Button>
              ...
              >
                {themeMode === "light" ? <Moon className="h-4 w-4" /> : <Sun className="h-4 w-4" />}
                <span className="text-xs">{themeMode === "light" ? "Dark" : "Light"}</span>
              </Button>
            </div>
