...
import { Plus, Trash2, Edit2, Check, X, Columns, FolderPlus, Folder, Sparkles } from "lucide-react"
...
interface CategoryBlockProps {
  ...
  onUpdateSubCategory: (categoryId: string, subCategoryId: string, updatedSubCategory: SubCategory) => void
...
export function CategoryBlock({
  ...
  const totalColumns =
    category.columns.length + category.subCategories.reduce((sum, sub) => sum + sub.columns.length, 0)
  ...
  const updateColumn = (columnId: string, updatedColumn: Column) => {
    onUpdate(category.id, {
      ...category,
      columns: category.columns.map((col) => (col.id === columnId ? updatedColumn : col)),
    })
  }
  ...
  const handleMoveColumn = (
    ...
    onMoveColumn(category.id, columnId, targetCategoryId, fromSubCategoryId, toSubCategoryId)
  }

  return (
    <Accordion type="single" collapsible defaultValue={isFirst ? category.id : undefined}>
      <AccordionItem value={category.id} className="border rounded-lg">
              ...
              {isEditing ? (
                ...
                <>
                  ...
                  <div className="flex flex-col">
                    ...
                    {category.namaEn && <span className="text-sm text-gray-500 italic">{category.namaEn}</span>}
                  </div>
        ...
        <AccordionContent className="px-4 pb-4">
          <div className="space-y-6">
                  ...
                  {category.subCategories.map((subCategory) => (
                    <SubCategoryBlock
                      ...
                      onUpdate={(updatedSubCategory) =>
                        onUpdateSubCategory(category.id, subCategory.id, updatedSubCategory)
                      }
            ...
            {/* Empty State */}
            {category.columns.length === 0 && category.subCategories.length === 0 && (
              <div className="text-center py-8 text-gray-500 bg-gray-50/50 rounded-lg border-2 border-dashed">
                ...
                <p className="text-sm">Tambah kolom langsung atau buat sub-kategori terlebih dahulu</p>
              </div>
            )}
          </div>
        </AccordionContent>
