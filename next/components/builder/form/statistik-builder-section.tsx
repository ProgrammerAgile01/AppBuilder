...
export function StatistikBuilderSection({
  statistikData,
  setStatistikData,
}: StatistikBuilderSectionProps) {
  const addStatistik = () => {
  ...
  const updateStatistik = (
    id: string,
    field: keyof StatistikData,
    value: string
  ) => {
    setStatistikData(
      statistikData.map((s) => (s.id === id ? { ...s, [field]: value } : s))
    );
  };
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Create dynamic statistics for your dashboard with custom queries
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <Button
          onClick={addStatistik}
          variant="outline"
...
function StatistikInputRow({
  statistik,
  onUpdate,
  onRemove,
}: StatistikInputRowProps) {
  const selectedIcon = ICON_OPTIONS.find(
    (icon) => icon.value === statistik.icon
  );
        ...
        <div className="flex-1 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  onUpdate(statistik.id, "judulStatistik", e.target.value)
                }
            ...
            <div className="space-y-2">
                    ...
                    if (statistik.judulStatistik) {
                      try {
                        const translation = await translateText(
                          statistik.judulStatistik
                        );
                        if (
                          translation &&
                          translation !== statistik.judulStatistik
                        ) {
                          onUpdate(
                            statistik.id,
                            "judulStatistikEn",
                          ...
                          toast({
                            ...
                            description:
                              "Could not translate title. Please enter manually.",
                            variant: "destructive",
                          });
                        }
                      } catch (error) {
              ...
              <Input
                ...
                onChange={(e) =>
                  onUpdate(statistik.id, "judulStatistikEn", e.target.value)
                }
                placeholder="Total Users"
                className="focus:ring-2 focus:ring-blue-500"
              />
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              onChange={(e) =>
                onUpdate(statistik.id, "queryAngka", e.target.value)
              }
          ...
          <div className="space-y-2">
            <Label className="text-sm font-medium">
              Query Resume Bulan Ini vs Bulan Lalu *
            </Label>
            <Textarea
              value={statistik.queryResume}
              onChange={(e) =>
                onUpdate(statistik.id, "queryResume", e.target.value)
              }
              placeholder="SELECT 
